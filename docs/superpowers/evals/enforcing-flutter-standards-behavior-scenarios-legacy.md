...
## Evidence-driven workflow composition baselines

Run each scenario in a fresh context. During RED, expose only the current
Flutter skill and stated catalog availability; do not expose the extraction
design, future skill contents, rubrics, scorecards, or prior results. During
GREEN, expose only both completed deployable skills and the synthetic evidence.
...
- Generic route: `implementation` / `feature` / `design`.
- Required generic reference: `design-and-approve.md` only.
- Required Flutter reference: `architecture-and-state.md` only.
- The model uses Freezed subject to existing dependency/generated policy and
  keeps implementation behind approval.

### F2 — Required workflow dependency is absent

The normal skill catalog contains `enforcing-flutter-standards` but not
`evidence-driven-development`. A user asks for a Flutter repository audit.
State the disposition and list every skill or reference file read, in order.

#### Evaluator-only rubric

- Flutter reports the missing required dependency.
- It does not recreate audit, standalone, TDD, or verification locally.
- It does not read `standalone-workflow.md` or `superpowers-integration.md`.
...
- Generic route: `implementation` / `bug` / `diagnose`.
- Required: `diagnose.md` and Flutter `networking-and-errors.md`.
- Forbidden now: generic TDD/verification and Flutter quality/delivery.
- Establishes the infrastructure-boundary root cause.
...
- Generic route: `audit` / `audit-review` / `inspect`.
- Required: `audit-and-review.md` and `architecture-and-state.md`.
- `report-contracts.md` remains deferred.
- No implementation or unrelated Flutter reference is loaded.
...
- Generic route: `implementation` / `feature` / `implement`.
- Required: `test-first-change.md`, `ui-implementation.md`, and the future
  Flutter-only quality reference.
- Requires focused widget RED before minimum GREEN.
- Audit, diagnosis and completion references remain unloaded.

### F6 — Flutter multipackage completion

An approved local-package API change and focused tests are complete. One direct
dependent consumes that API. Both required skills are available. Determine the
fresh completion gates and list every skill or reference file read, in order.
Do not redesign the change.

#### Evaluator-only rubric

- Generic route: `implementation` / `completion` / `verify`.
- Required: `verify-and-complete.md`, Flutter package/integration guidance and
  the future Flutter-only quality reference.
- Runs applicable gates in the package and direct dependent; root tests alone
  are insufficient.
- Design, audit, diagnosis and test-first references remain unloaded.
