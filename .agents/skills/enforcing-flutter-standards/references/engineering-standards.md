...
| Conditional | Extract a local package only for an actual stable architectural or anti-corruption boundary. Reuse, a small owned API, lock-in isolation, and isolated tests support the decision; none alone mandates extraction. |
...
| A new-app environment batch or an already approved scoped change adds or migrates required configuration, or evidence shows an existing setup is not coherent or safe because missing/invalid required values can reach runtime. | Represent required values with explicit types, validate them at startup, and fail with an actionable message. Never silently fall back to production. | New-app validation is included only in its approved environment batch. Existing-project validation requires the approved named configuration/remediation batch; choosing a product default requires a product decision. | State which predicate applied, required values and types, safety evidence when applicable, approved batch/product decision, validation behavior, and tests for missing/invalid values. |
...
| SDK | Significant integration reaches feature/state | Owned service/adapter and owned types; package only for an approved stable anti-corruption boundary. |
